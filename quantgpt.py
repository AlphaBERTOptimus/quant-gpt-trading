...
with col1:
    user_input = st.text_input(
        "Command",
        placeholder="Enter: AAPL, screen stocks, backtest TSLA, compare NVDA vs AAPL...",
        key="terminal_input",
...
if execute_btn and user_input.strip():
    # Check for custom screening criteria
    if "screen stocks with custom criteria" in user_input and st.session_state.custom_screening_criteria:
        # Override the screener's default criteria
        st.session_state.analyzer.screener.custom_criteria = st.session_state.custom_screening_criteria
...
"""
            screening_results = update["content"]
            
        elif update["type"] == "backtest_results":
            results = update["content"]
            result_content = f"""
## 📈 Backtest Results - {results['strategy']}

### 📊 Performance Summary
- **Strategy Return:** {results['total_return']:.1f}%
- **Buy & Hold Return:** {results['buy_hold_return']:.1f}%  
...
"""
            ...
            if results['total_return'] > results['buy_hold_return']:
                result_content += "✅ **Strategy outperformed buy-and-hold**\n\n"
            else:
                result_content += "❌ **Strategy underperformed buy-and-hold**\n\n"
            ...
            if screening_results is not None:
                final_message["screening_results"] = screening_results
            if backtest_results is not None:
                final_message["backtest_results"] = backtest_results
...
<div style='text-align: center; color: var(--text-secondary); padding: 2rem; font-family: "Inter", sans-serif;'>
    ...
    <p>Enhanced with Stock Screener • Strategy Backtesting • AI-Powered Analysis</p>
    <p style="font-size: 0.75rem; margin-top: 1rem;">
        🔍 Screen 500+ stocks • 📈 Backtest strategies • 🤖 AI insights • ⚠️ Educational use only
    </p>
</div>
""", unsafe_allow_html=True)
