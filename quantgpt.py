    ...
    def get_all_us_stocks(self) -> pd.DataFrame:
        ...
        try:
            ...
            for exchange in exchanges:
                try:
                    # Using yfinance to get stock lists
                    if exchange == 'NYSE':
                        # Sample of major NYSE stocks
                        nyse_stocks = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'BRK-B', 
                                     'JPM', 'JNJ', 'V', 'PG', 'UNH', 'HD', 'MA', 'DIS', 'PYPL', 'BAC',
                                     'ADBE', 'CRM', 'NFLX', 'KO', 'PFE', 'INTC', 'VZ', 'WMT', 'T', 'XOM',
                                     'CVX', 'MRK', 'ABT', 'COST', 'PEP', 'TMO', 'ACN', 'AVGO', 'TXN', 'LLY',
                                     'NKE', 'MDT', 'DHR', 'UPS', 'QCOM', 'NEE', 'PM', 'IBM', 'LIN', 'LOW',
                                     'HON', 'AMGN', 'SBUX', 'UNP', 'C', 'GS', 'SPGI', 'BLK', 'CAT', 'ISRG',
                                     'AXP', 'BKNG', 'GILD', 'MMM', 'SYK', 'TJX', 'CVS', 'MO', 'USB', 'ADP',
                                     'MDLZ', 'TMUS', 'CI', 'SO', 'ZTS', 'DUK', 'CL', 'GE', 'D', 'ICE']
                        for symbol in nyse_stocks:
                            all_stocks.append({'Symbol': symbol, 'Exchange': 'NYSE'})
                    
                    elif exchange == 'NASDAQ':
                        # Sample of major NASDAQ stocks
                        nasdaq_stocks = ['QQQ', 'SPY', 'IWM', 'ARKK', 'AMD', 'MRNA', 'ZM', 'DOCU', 'ROKU',
                                       'PTON', 'SQ', 'SHOP', 'TWLO', 'OKTA', 'SNOW', 'PLTR', 'RBLX', 'HOOD',
                                       'COIN', 'RIVN', 'LCID', 'F', 'GM', 'UBER', 'LYFT', 'ABNB', 'DASH',
                                       'DKNG', 'PENN', 'FUBO', 'CRSR', 'CRWD', 'ZS', 'NET', 'DDOG', 'ESTC',
                                       'MDB', 'TEAM', 'WDAY', 'SPLK', 'NOW', 'PANW', 'FTNT', 'CYBR', 'OKTA']
                        for symbol in nasdaq_stocks:
                            all_stocks.append({'Symbol': symbol, 'Exchange': 'NASDAQ'})
            ...
            if all_stocks:
                ...
                fallback_stocks = [
                    ...
                    'PFE', 'INTC', 'VZ', 'WMT', 'T', 'XOM', 'CVX', 'MRK', 'ABT', 'COST', 'PEP', 'TMO'
                ]
                stocks_df = pd.DataFrame([{'Symbol': s, 'Exchange': 'Mixed'} for s in fallback_stocks])
                self.stocks_cache = stocks_df
                self.last_update = current_time
                return stocks_df
...
"""
            ...
            if fund.get('roe'): 
                ...
                fund_data.append(["ROE", f"{fund['roe']*100:.1f}%", roe_signal])
            ...
            if fund.get('revenue_growth'): fund_data.append(["Revenue Growth", f"{fund['revenue_growth']*100:.1f}%", ""])
            ...
            for symbol in symbols:
                if symbol in analyses:
                    ...
                    comp_data.append({
                        ...
                        symbol: f"${analysis['price']:.2f}"
                    })
                    comp_data.append({
                        "Metric": "Daily Change",
                        symbol: f"{analysis['change']:+.2f}%"
                    })
                    comp_data.append({
                        "Metric": "Market Cap",
                        symbol: f"${(analysis.get('market_cap', 0) or 0)/1e9:.1f}B" if analysis.get('market_cap') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "P/E Ratio",
                        symbol: f"{fund.get('pe_ratio', 0):.1f}" if fund.get('pe_ratio') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "RSI",
                        symbol: f"{tech.get('rsi', 0):.1f}" if tech.get('rsi') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "ROE",
                        symbol: f"{(fund.get('roe', 0) or 0)*100:.1f}%" if fund.get('roe') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "YTD Return",
                        symbol: f"{perf.get('ytd_return', 0):+.1f}%" if perf.get('ytd_return') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "Volatility",
                        symbol: f"{perf.get('volatility', 0):.1f}%" if perf.get('volatility') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "Beta",
                        symbol: f"{fund.get('beta', 0):.2f}" if fund.get('beta') else 'N/A'
                    })
                    comp_data.append({
                        "Metric": "Sector",
                        symbol: analysis.get('sector', 'Unknown')[:20]
                    })
            
            # Restructure data for comparison table
            if comp_data and len(symbols) >= 2:
                metrics = list(set([item["Metric"] for item in comp_data]))
                comparison_data = []
                
                for metric in metrics:
                    row = {"Metric": metric}
                    for symbol in symbols:
                        values = [item[symbol] for item in comp_data if item["Metric"] == metric and symbol in item]
                        row[symbol] = values[0] if values else 'N/A'
